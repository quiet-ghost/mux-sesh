...
export function handleModalKeyboard(key: KeyboardInput, ctx: ModalKeyboardContext): boolean {
  ...
  if (ctx.modalState?.type === 'commands') {
    if (key.name === 'escape' || key.name === 'q') {
      ...
      ctx.setCommandsCursor(current => Math.min(current + 1, ctx.filteredCommandEntries.length - 1))
    } else if (key.name === 'up' || key.name === 'k') {
      ctx.setCommandsCursor(current => Math.max(current - 1, 0))
  ...
  if (ctx.modalState?.type === 'settings') {
    if (key.name === 'escape') {
      ...
      ctx.setSettingsCursor(current =>
        Math.min(current + 1, ctx.filteredSettingsEntries.length - 1)
      )
    } else if (key.name === 'up' || key.name === 'k') {
      ctx.setSettingsCursor(current => Math.max(current - 1, 0))
  ...
  if (ctx.modalState?.type === 'setting-options') {
    if (key.name === 'escape') {
      ...
      ctx.setSettingOptionsCursor(current =>
        Math.min(current + 1, ctx.filteredSettingOptions.length - 1)
      )
    } else if (key.name === 'up' || key.name === 'k') {
      ctx.setSettingOptionsCursor(current => Math.max(current - 1, 0))
    } else if (key.name === 'return') {
